...
3. **Custom `fetch` Calls** (non-Inertia/Axios):
   - Must explicitly include `credentials: 'same-origin'`
   - Set `Accept: 'application/json'` and `X-Requested-With: 'XMLHttpRequest'`
   - Read meta `csrf-token` (or `router.page.props.csrf_token`) and send `X-CSRF-TOKEN`
   - Prefer centralized helpers/axios when possible to avoid regressions

### Hardened Endpoints (Nov 2025)

- `resources/js/Components/KanbanBoard.jsx` – Kanban reorder payload
- `resources/js/Components/EisenhowerMatrix.jsx` – Eisenhower reorder payload
- `resources/js/Components/AttachmentList.jsx` – Attachment deletion

Future custom `fetch` implementations should follow the above checklist before merging.
